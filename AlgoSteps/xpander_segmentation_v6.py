...
# All debug/detection outlines in this module are drawn at half of the
# previous thickness. Keeping the scale in one place makes future changes
# consistent across the complete Stage-6 visualization.
DETECTION_LINEWIDTH_SCALE: float = 0.5


def _detection_linewidth(base_width: float) -> float:
    return float(base_width * DETECTION_LINEWIDTH_SCALE)
...
class XpanderSegmentationConfig:
    ...
    # Full physical Xpander mask.
    #
    # The four detected corners define a reliable geometric CORE.  The final
    # physical label is then grown from that core so connected corner blocks
    # (including the small square used during corner localization) are not
    # discarded.  A broad height envelope preserves gradual curvature, while
    # a gradient barrier and a small morphological bridge prevent the region
    # from freely leaking through strong component boundaries.
    full_mask_enabled: bool = True
    full_mask_horizontal_margin_fraction: float = 0.20
    full_mask_vertical_margin_fraction: float = 0.20
    full_mask_minimum_margin_pixels: int = 12
    full_mask_seed_erosion_pixels: int = 3
    full_mask_height_tolerance_fraction: float = 0.12
    full_mask_height_tolerance_absolute: float = 0.15
    full_mask_core_quantile_low: float = 1.0
    full_mask_core_quantile_high: float = 99.0
    full_mask_gradient_barrier_fraction: float = 0.018
    full_mask_gradient_barrier_absolute: float = 0.08
    full_mask_core_gradient_percentile: float = 97.5
    full_mask_core_gradient_multiplier: float = 2.0
    full_mask_bridge_halo_pixels: int = 2
    full_mask_boundary_recovery_pixels: int = 1
    full_mask_closing_iterations: int = 1

    # The bottom threshold transition is the small corner block used to locate
    # the Xpander corner.  It is physically part of the Xpander even though the
    # conservative core BB stops at its inner edge.  Preserve that block
    # explicitly, using its detected threshold height and width.
    full_mask_include_corner_blocks: bool = True
    full_mask_corner_block_depth_factor: float = 1.25
    full_mask_corner_block_minimum_depth_pixels: int = 2
    full_mask_corner_height_tolerance_fraction: float = 0.04
    full_mask_corner_height_tolerance_absolute: float = 0.08
...
class XpanderSegmentationResult:
    ...
    ``core_mask`` is the conservative quadrilateral obtained directly from
    the four detected corners. ``xpander_mask`` is the complete physical mask
    grown from that core and explicitly includes the detected raised corner
    squares, plus other connected genuine Xpander surface just outside the
    core BB.
    """
    ...
    # Conservative geometry used as the seed for the full result.
    core_bounding_box: BoundingBox
    core_mask: BoolArray
    xpander_reference_height: float
    full_mask_height_tolerance: float
    full_mask_added_pixels: int
...
def segment_xpander(
    ...
    core_mask = _polygon_to_mask(
        image_shape=height_map.shape,
        polygon_points=(top_left, top_right, bottom_right, bottom_left),
    )
    core_mask &= ~pivot_segmentation.pivot_mask

    core_bounding_box = _bounding_box_from_mask(
        core_mask,
        empty_message="The detected Xpander polygon produced an empty core mask.",
    )

    xpander_reference_height = float(
        np.median(
            [
                left_side.xpander_height,
                right_side.xpander_height,
                left_top.diagnostics.stable_height,
                right_top.diagnostics.stable_height,
            ]
        )
    )

    if config.full_mask_enabled:
        (
            xpander_mask,
            full_mask_height_tolerance,
        ) = _grow_full_xpander_mask(
            smoothed=smoothed,
            gradient_x=gradient_x,
            gradient_y=gradient_y,
            core_mask=core_mask,
            core_bounding_box=core_bounding_box,
            pivot_mask=pivot_segmentation.pivot_mask,
            left_side_detection=left_side,
            right_side_detection=right_side,
            xpander_reference_height=xpander_reference_height,
            robust_height_range=robust_height_range,
            config=config,
        )
    else:
        xpander_mask = core_mask.copy()
        full_mask_height_tolerance = 0.0

    # The geometric core is always retained.  Expansion is allowed to add
    # information, never to erase a region already accepted by the detector.
    xpander_mask |= core_mask
    xpander_mask &= ~pivot_segmentation.pivot_mask

    bounding_box = _bounding_box_from_mask(
        xpander_mask,
        empty_message="The final physical Xpander mask is empty.",
    )
    full_mask_added_pixels = int(
        np.count_nonzero(xpander_mask & ~core_mask)
    )
    ...
    return XpanderSegmentationResult(
        ...
        core_mask=core_mask,
        xpander_reference_height=xpander_reference_height,
        full_mask_height_tolerance=full_mask_height_tolerance,
        full_mask_added_pixels=full_mask_added_pixels,
        bottom_left=bottom_left,
        bottom_right=bottom_right,
        top_left=top_left,
        top_right=top_right,
...
def _bounding_box_from_mask(
    ...
    return BoundingBox(
        ...
        x_max=int(x_coordinates.max()) + 1,
        y_max=int(y_coordinates.max()) + 1,
    )


def _grow_full_xpander_mask(
    smoothed: NDArray[np.float32],
    gradient_x: NDArray[np.float32],
    gradient_y: NDArray[np.float32],
    core_mask: BoolArray,
    ...
    """
    Grow a complete physical Xpander label from the conservative core.

    The purpose of this step is information preservation.  The corner blocks
    used by the geometric detector can lie just outside the four-corner core,
    but still belong physically to the Xpander and may be important for later
    contour/curvature analysis.

    The growth is intentionally constrained by four independent safeguards:

    1. A local ROI around the detected core.
    2. A broad height envelope that permits gradual curved surfaces.
    3. A gradient barrier that blocks strong component boundaries.
    4. Connectivity to an eroded core seed.

    The small morphological bridge is specifically intended to cross the
    stable shadow/halo surrounding the component without treating the halo as
    the final Xpander boundary.
    """
    image_height, image_width = smoothed.shape

    horizontal_margin = max(
        config.full_mask_minimum_margin_pixels,
        int(
            round(
                core_bounding_box.width
                * config.full_mask_horizontal_margin_fraction
            )
        ),
    )
    vertical_margin = max(
        ...
        int(
            round(
                core_bounding_box.height
                * config.full_mask_vertical_margin_fraction
            )
        ),
    )

    roi_box = BoundingBox(
        x_min=max(0, core_bounding_box.x_min - horizontal_margin),
        y_min=max(0, core_bounding_box.y_min - vertical_margin),
        x_max=min(image_width, core_bounding_box.x_max + horizontal_margin),
        y_max=min(image_height, core_bounding_box.y_max + vertical_margin),
    )

    roi_mask = np.zeros(smoothed.shape, dtype=bool)
    roi_mask[
        roi_box.y_min:roi_box.y_max,
        roi_box.x_min:roi_box.x_max,
    ] = True

    erosion_iterations = max(0, config.full_mask_seed_erosion_pixels)
    if erosion_iterations > 0:
        interior_core = ndi.binary_erosion(
            core_mask,
        ...
        interior_core = core_mask.copy()

    if not np.any(interior_core):
        interior_core = core_mask.copy()

    interior_values = smoothed[interior_core]
    if interior_values.size == 0:
        raise ValueError(
            "Cannot grow the full Xpander mask from an empty core seed."
        )

    quantile_low = float(
        np.percentile(
            interior_values,
            config.full_mask_core_quantile_low,
        )
    )
    quantile_high = float(
        np.percentile(
            interior_values,
            config.full_mask_core_quantile_high,
        )
    )

    # The envelope is deliberately broad.  It should preserve a curved
    # Xpander surface, not force the entire component to be perfectly flat.
    height_tolerance = max(
        config.full_mask_height_tolerance_absolute,
        config.full_mask_height_tolerance_fraction * robust_height_range,
        abs(xpander_reference_height - quantile_low),
        abs(quantile_high - xpander_reference_height),
    )
    minimum_height = min(
        quantile_low,
        xpander_reference_height - height_tolerance,
    )
    maximum_height = max(
        quantile_high,
        xpander_reference_height + height_tolerance,
    ...
    gradient_magnitude = np.hypot(gradient_x, gradient_y)
    interior_gradient = gradient_magnitude[interior_core]
    if interior_gradient.size:
        core_gradient_level = float(
            np.percentile(
                interior_gradient,
                config.full_mask_core_gradient_percentile,
            )
        )
    else:
        core_gradient_level = 0.0

    gradient_barrier = max(
        config.full_mask_gradient_barrier_absolute,
        config.full_mask_gradient_barrier_fraction * robust_height_range,
        config.full_mask_core_gradient_multiplier * core_gradient_level,
    )

    broad_height_mask = (
        (smoothed >= minimum_height)
        & (smoothed <= maximum_height)
    )
    traversable_mask = (
        roi_mask
        & broad_height_mask
        & (gradient_magnitude <= gradient_barrier)
        & ~pivot_mask
    )

    # A narrow, stable halo can split one physical component into two binary
    # components.  Closing only inside the constrained ROI bridges that halo.
    bridge_iterations = max(0, config.full_mask_bridge_halo_pixels)
    if bridge_iterations > 0:
        traversable_mask = ndi.binary_closing(
            traversable_mask,
            structure=np.ones((3, 3), dtype=bool),
            iterations=bridge_iterations,
            border_value=0,
        )
        traversable_mask &= roi_mask
        traversable_mask &= broad_height_mask
        traversable_mask &= ~pivot_mask

    # Use only stable interior pixels as seeds, so the geometric core cannot
    # itself act as a bridge across an unrelated strong edge.
    seed_mask = interior_core & traversable_mask
    if not np.any(seed_mask):
        seed_mask = core_mask & broad_height_mask & roi_mask & ~pivot_mask

    if not np.any(seed_mask):
        # Safe fallback: preserve the core rather than returning no label.
        return core_mask.copy(), float(height_tolerance)
    ...
    touching_labels = np.unique(component_labels[seed_mask])
    touching_labels = touching_labels[touching_labels != 0]

    if touching_labels.size == 0:
        return core_mask.copy(), float(height_tolerance)

    grown_mask = np.isin(component_labels, touching_labels)

    # Restore a thin physical boundary that may have been excluded by the
    # gradient barrier.  Recovery remains inside the same broad height/ROI
    # constraints and therefore does not blindly dilate into neighboring parts.
    recovery_iterations = max(0, config.full_mask_boundary_recovery_pixels)
    if recovery_iterations > 0:
        recovered_boundary = ndi.binary_dilation(
            grown_mask,
            structure=np.ones((3, 3), dtype=bool),
            iterations=recovery_iterations,
        )
        recovered_boundary &= roi_mask
        recovered_boundary &= broad_height_mask
        recovered_boundary &= ~pivot_mask
        grown_mask |= recovered_boundary

    closing_iterations = max(0, config.full_mask_closing_iterations)
    if closing_iterations > 0:
        grown_mask = ndi.binary_closing(
            grown_mask,
            structure=np.ones((3, 3), dtype=bool),
        ...
        grown_mask &= roi_mask
        grown_mask &= broad_height_mask
        grown_mask &= ~pivot_mask

    if config.full_mask_include_corner_blocks:
        corner_blocks = _build_detected_corner_blocks_mask(
            image_shape=smoothed.shape,
            smoothed=smoothed,
            pivot_mask=pivot_mask,
            left_side_detection=left_side_detection,
            right_side_detection=right_side_detection,
            robust_height_range=robust_height_range,
            config=config,
        )
        grown_mask |= corner_blocks

    grown_mask |= core_mask
    grown_mask &= ~pivot_mask
    return grown_mask.astype(bool, copy=False), float(height_tolerance)


def _build_detected_corner_blocks_mask(
    image_shape: tuple[int, int],
    smoothed: NDArray[np.float32],
    pivot_mask: BoolArray,
    left_side_detection: SideCornerDetection,
    ...
    """
    Preserve the physical corner squares identified by the bottom transitions.

    The conservative core boundary is the inner Xpander-to-threshold edge.
    The raised block immediately outside that edge was used to localize the
    corner and belongs to the complete Xpander.  Its horizontal extent is
    measured directly by the rise/fall pair.  Because the feature is described
    as a corner square, its vertical depth is derived from that measured width.

    Only pixels compatible with the measured threshold height are retained, so
    the rectangular support window does not automatically label background.
    """
    image_height, image_width = image_shape
    blocks = np.zeros(image_shape, dtype=bool)

    for detection in (left_side_detection, right_side_detection):
        transition = detection.transition

        x_start = max(
            0,
            min(transition.rise_position, transition.fall_position),
        )
        x_stop = min(
            image_width,
            max(transition.rise_position, transition.fall_position) + 1,
        )
        measured_width = max(1, x_stop - x_start)

        block_depth = max(
            config.full_mask_corner_block_minimum_depth_pixels,
            int(
                round(
                    measured_width
                    * config.full_mask_corner_block_depth_factor
                )
            ),
        )

        # The core extends upward from the detected bottom corner.  The corner
        # block lies outside the core on the lower side of that boundary.
        y_start = int(np.clip(detection.bottom_corner.y, 0, image_height))
        y_stop = min(image_height, y_start + block_depth)
        if x_stop <= x_start or y_stop <= y_start:
            continue

        height_tolerance = max(
            config.full_mask_corner_height_tolerance_absolute,
            config.full_mask_corner_height_tolerance_fraction
            * robust_height_range,
        )
        local_values = smoothed[y_start:y_stop, x_start:x_stop]
        local_match = (
            np.abs(local_values - transition.threshold_height)
            <= height_tolerance
        )

        if np.any(local_match):
            # Close tiny holes caused by the shadow/halo or measurement noise.
            local_match = ndi.binary_closing(
                local_match,
                structure=np.ones((3, 3), dtype=bool),
                iterations=1,
                border_value=1,
            )
            blocks[y_start:y_stop, x_start:x_stop] |= local_match
...
def print_xpander_segmentation(result: XpanderSegmentationResult) -> None:
    ...
    print("-" * 100)
    print(f"Full physical bounding box: {result.bounding_box}")
    print(f"Geometric core bounding box: {result.core_bounding_box}")
    print(
        "Full-mask expansion: "
        f"added_pixels={result.full_mask_added_pixels}, "
        f"reference_height={result.xpander_reference_height:.6f}, "
        f"height_tolerance={result.full_mask_height_tolerance:.6f}"
    )
    print(
        "Corners: "
...
def plot_xpander_segmentation(
    ...
    core_mask_crop = result.core_mask[
        crop_box.y_min:crop_box.y_max,
        crop_box.x_min:crop_box.x_max,
    ]
    ...
    geometry_axis.add_patch(
        Polygon(
            ...
            fill=False,
            linestyle="--",
            linewidth=_detection_linewidth(3.0),
    ...
    geometry_axis.contour(
        ...
        levels=[0.5],
        linewidths=_detection_linewidth(3.0),
    )
    ...
    geometry_axis.add_patch(
        Rectangle(
            ...
            fill=False,
            linestyle="--",
            linewidth=_detection_linewidth(2.0),
            label="Pivot",
        )
    )
    ...
    for point, label in (
        ...
        geometry_axis.scatter(
            ...
            marker="x",
            s=80,
            linewidths=_detection_linewidth(2.5),
        )
    ...
    geometry_axis.set_title("Stage 6 — geometric core and full physical Xpander")
    ...
    mask_axis.contour(
        core_mask_crop.astype(np.uint8),
        levels=[0.5],
        linestyles="--",
        linewidths=_detection_linewidth(2.0),
    )
    mask_axis.set_title(
        "Full physical Xpander label (core shown dashed)\n"
        f"added={result.full_mask_added_pixels}px, "
...
def _plot_profile(
    ...
    axis.plot(
        ...
        diagnostics.profile,
        label=label,
        linewidth=_detection_linewidth(1.5),
    )
    axis.axvspan(
...
def _validate_inputs(
    ...
    if config.full_mask_minimum_margin_pixels < 0:
        raise ValueError("full_mask_minimum_margin_pixels cannot be negative.")
    if config.full_mask_seed_erosion_pixels < 0:
        raise ValueError("full_mask_seed_erosion_pixels cannot be negative.")
    if config.full_mask_bridge_halo_pixels < 0:
        raise ValueError("full_mask_bridge_halo_pixels cannot be negative.")
    if config.full_mask_boundary_recovery_pixels < 0:
        raise ValueError("full_mask_boundary_recovery_pixels cannot be negative.")
    if config.full_mask_corner_block_depth_factor <= 0:
        raise ValueError("full_mask_corner_block_depth_factor must be positive.")
    if config.full_mask_corner_block_minimum_depth_pixels < 1:
        raise ValueError(
            "full_mask_corner_block_minimum_depth_pixels must be at least 1."
        )
    if not (0.0 <= config.full_mask_core_quantile_low < config.full_mask_core_quantile_high <= 100.0):
        raise ValueError("Full-mask core quantiles must satisfy 0 <= low < high <= 100.")
