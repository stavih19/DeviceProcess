...
# prompt:
# תכין setup ראשוני לקריאת קבצים מהסוג שדיברנו עליהם מתיקייה שניתנת כםרמטר בפקודה הרצת הקובת main.py
# הכוונה לקוד
# מריצה את האלגוריתם (מספיק רק קריאה לפונקציה ללא מימוש - נטו setup) שבסופו מוציאה אובייקט המכיל את כל הפלטים שצריך לענות בשאלה
# מלבד הנתונים היבשים יש לנו גם את התמונה label של שני הרכבים
# תשמור באובייקט רק את השם של הקובץ ואת כל הקבצים המהווים פלט לכל קלט תשמור בתיקייה נפרדת מהקלטים